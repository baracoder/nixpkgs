...
unpackFile() {
    # The first part of the installer file is a shell script.
    # For debugging, use: sh -x $src -x
    sh $src -x
    sourceRoot=NVIDIA-Linux-x86_64-${version}
}
...
installPhase() {
    ...
    # https://github.com/NVIDIA/nvidia-installer/blob/3d6335fc8be548eac5d0c654b318a856a26871dd/install-from-cwd.c#L721
    # skip first 8 lines
    tail -n+9 .manifest | while read FILENAME MASK TYPE ARG4 ARG5 ARG6; do
        BASENAME=$(basename $FILENAME)
        # special handling for files
        case $FILENAME in
            lib{glx,nvidia-wfb}.so.*)
                echo "Ignoring $FILENAME: handled separately"
                ;;
            libOpenCL.so*)
                echo "Ignoring $FILENAME: Use ocl-icd instead"
                continue
                ;;
            libnvidia-gtk*)
                echo "Ignoring $FILENAME: Built from source"
                continue
                ;;
            lib{GL,GLX,EGL,GLESv1_CM,GLESv2,OpenGL,GLdispatch}.so.*)
                if [ "$useGLVND" = "true" ]; then
                    continue
                fi
                ;;
        esac
        ...
        case $TYPE in
            APPLICATION_PROFILE)
                install -D --mode=$MASK $FILENAME $out/share/nvidia/$BASENAME
                ;;
            UTILITY_BINARY)
                install -D --mode=$MASK $FILENAME $out/bin/$BASENAME
                ;;
            SYSTEMD_UNIT)
                install -D --mode=$MASK $FILENAME $out/$FILENAME
                ;;
            ...
            *_LIB)
                case $ARG4 in
                    NATIVE)
                        install -m$MASK $FILENAME $out/lib/
                        ;;
                    COMPAT32)
                        install -m$MASK $FILENAME $lib32/lib/
                        ;;
                esac
                ;;
            *_LIB_SYMLINK)
                case $ARG4 in
                    NATIVE)
                        ln -s $out/lib/$ARG5 $out/lib/$FILENAME
                        ;;
                    COMPAT32)
                        ln -s $lib32/lib/$ARG5 $lib32/lib/$FILENAME
                        ;;
                esac
                ;;
            FIRMWARE)
            ...
            DOCUMENTATION)
                ...
                ;;
            *)
                echo "Warning: unhandled file $FILENAME $TYPE"
                ;;
        esac
    done

    sed -E 's#(PATH=).*#\1"$PATH"#' -i $out/bin/nvidia-sleep.sh 
